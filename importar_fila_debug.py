...
for _, row in df.iterrows():
    try:
        duracao = parse_duracao(row.get('Duração'))
        ...
        """, (reg_id, nome, pessoas, dia_ch, hora_ch, dia_fin, hora_fin, duracao, 'Restaurant', None, 'Report_Espera_YTD_Jun16.xls'))
...
cur.close()
conn.close()
print(f'Inseridos: {ins} | Duplicatas: {dup} | Erros: {len(erros)}')
if erros:
    print('Primeiro erro:', erros[0])
