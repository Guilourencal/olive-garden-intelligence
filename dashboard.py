    ...
    if len(df_menu) == 0:
        ...
        with st.container(border=True):
            ...
            if pd.notna(dog_check_avg) and pd.notna(star_check_avg):
                st.markdown(
                    f'<div style="background:#3D2B1F; border-radius:8px; padding:12px 16px; margin-bottom:16px; border-left:4px solid #B8923A;">'                    f'<span style="font-size:12px; color:#F5F0E8;">💡 <b>Insight:</b> Dogs tem check completo medio '                    f'<b style="color:#B8923A;">R$ {dog_check_avg:.0f}</b> — maior que Stars '                    f'(<b>R$ {star_check_avg:.0f}</b>). O problema e visibilidade, nao o cliente.</span></div>',
                    unsafe_allow_html=True
                )
        ...
        with st.container(border=True):
            ...
            with col_t4b:
                ...
                st.markdown(
                    f'<div style="text-align:center; padding:20px;">'                    f'<div style="font-size:48px; font-weight:800; color:{cor_gauge};">{qty_atual:.2f}</div>'                    f'<div style="font-size:12px; color:#8B7A5A; margin:8px 0;">un/check atual</div>'                    f'<div style="background:#e8ddc8; border-radius:8px; height:12px; margin:12px 0;">'                    f'<div style="background:{cor_gauge}; width:{max(pct_gauge,5):.0f}%; height:12px; border-radius:8px;"></div></div>'                    f'<div style="display:flex; justify-content:space-between; font-size:10px; color:#8B7A5A;">'                    f'<span>Baseline: {baseline}</span><span>Meta: {meta}</span></div></div>',
                    unsafe_allow_html=True
                )
            ...
            for col, canal_t5 in [(col_pos, "POS"), (col_dlv, "Delivery")]:
                ...
                with col:
                    ...
                    for _, r in top5.iterrows():
                        ...
                        st.markdown(
                            f'<div style="display:flex; justify-content:space-between; padding:4px 0; border-bottom:1px solid #e8ddc8;">'                            f'<span style="font-size:10px; color:#3D2B1F;">{r["item"][:28]}</span>'                            f'<span style="font-size:10px; font-weight:700; color:{cor};">{r["type"]}</span></div>',
                            unsafe_allow_html=True
                        )
                    # Hero KPI Delivery
                    if canal_t5 == "Delivery":
                        ...
                        if pd.notna(puzz_dlv):
                            st.markdown(
                                f'<div style="background:#1a1209; border-radius:8px; padding:12px; margin-top:12px; border:1px solid #8B9A2E; text-align:center;">'                                f'<div style="font-size:9px; color:#8B9A2E; letter-spacing:2px;">GUEST AVG PUZZLES DELIVERY</div>'                                f'<div style="font-size:32px; font-weight:800; color:#8B9A2E;">R$ {puzz_dlv:.0f}</div>'                                f'<div style="font-size:9px; color:#8B7A5A;">maior do portfolio</div></div>',
                                unsafe_allow_html=True
                            )
