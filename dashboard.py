...
if aba_sel == "Reviews":
    ...
    st.markdown('''<div style="font-weight:800; font-size:26px; color:#3D2B1F; letter-spacing:0.08em; text-transform:uppercase; margin-bottom:4px;">Vendas</div><div style="font-size:13px; color:#8B9A2E; letter-spacing:0.1em; margin-bottom:20px;">PERFORMANCE FINANCEIRA — iFOOD</div>''', unsafe_allow_html=True)
    ...
    def fmt_periodo(p):
        try: return mes_map.get(p.split("/")[1].strip()[:2], p)
        except: return p
    ...
    with st.container(border=True):
        ...
        for idx, periodo in enumerate(periodos):
            ...
            delta = f" (+{((fat/prev_fat-1)*100):.1f}%)" if prev_fat and prev_fat > 0 else ""
            prev_fat = fat
            with cols_v[idx]:
                st.markdown(f'''<div style="background:#3D2B1F; border-radius:12px; padding:20px; color:#F5F0E8; margin-bottom:8px;">
                    <div style="font-size:10px; letter-spacing:3px; color:#8B9A2E; text-transform:uppercase; margin-bottom:14px;">{fmt_periodo(periodo)}{delta}</div>
                    <div style="display:grid; grid-template-columns:1fr 1fr; gap:14px;">
                    <div><div style="font-size:9px; color:#D8CFC0; margin-bottom:4px;">FATURAMENTO</div><div style="font-size:18px; font-weight:700;">R</div></div>
                    <div><div style="font-size:9px; color:#D8CFC0; margin-bottom:4px;">PEDIDOS</div><div style="font-size:18px; font-weight:700;">{ped:,}</div></div>
                    <div><div style="font-size:9px; color:#D8CFC0; margin-bottom:4px;">TICKET MEDIO</div><div style="font-size:18px; font-weight:700; color:#8B9A2E;">R</div></div>
                    <div><div style="font-size:9px; color:#D8CFC0; margin-bottom:4px;">NOVOS CLIENTES</div><div style="font-size:18px; font-weight:700; color:#8B9A2E;">{nov:,}</div></div>
    ...
    with st.container(border=True):
        ...
        df_rank["ticket_medio"] = df_rank["faturamento"] / df_rank["pedidos"]
        ...
        fig_rank.update_layout(paper_bgcolor="rgba(0,0,0,0)", plot_bgcolor="rgba(0,0,0,0)", margin=dict(t=10,b=10,l=10,r=120), xaxis=dict(showgrid=False, tickfont=dict(family="Nunito", size=11, color=MARROM)), yaxis=dict(tickfont=dict(family="Nunito", size=12, color=MARROM)), font=dict(family="Nunito"), height=280)
    ...
    with col_h2:
        with st.container(border=True):
            ...
            df_dias_g = df_dias_g.set_index("dia_norm").reindex([d for d in ordem_dias if d in df_dias_g["dia_norm"].values]).reset_index()
            if len(df_dias_g) > 0:
